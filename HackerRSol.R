...
answer <- "NO"
if (((data[3] - data[1])/(data[2] - data[4]))%%1 == 0 && ((data[3] - data[1])/(data[2] - data[4])) > 0 && (data[2] != 
    data[4])) {
    answer <- "YES"
}
...
for (i in 2:(msize - 1)) {
    for (j in 2:(msize - 1)) {
        if ((cgrid[i, j]) > (cgrid[i - 1, j]) && (cgrid[i, j]) > (cgrid[i + 1, j]) && (cgrid[i, j]) > (cgrid[i, 
            j - 1]) && (cgrid[i, j]) > (cgrid[i, j + 1])) {
            ogrid[i, j] <- "X"
        }
    }
}
